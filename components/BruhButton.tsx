...
export default function BruhButton({errorCode, weatherState, handleWeatherClick}:BruhButtonProps):JSX.Element {
  ...
  return (
    <div className="flex flex-row items-center
      ...
      <button className="bg-win-main border-3d hover:bg-win-accent px-3 py-2.5 leading-none font-bold text-xl text-white
        transition-[scale,background-color] duration-200 ease-in-out active:scale-95"
        onClick={errorCode === 1 ? () => {} : handleWeatherClick}
      >
      ...
      {/* Hover Message */}
      <HoverMessage mainMessage={mainMessage} subMessage={errorCode === 1 ? "Please allow and refresh page" : "Click to retry"} theme="light" />
    </div>
  )
}
